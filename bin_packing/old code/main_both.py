...
1. This heuristic serves is used in the <|evaluation function|>.  
2. The heuristic contains two functions. A `priority` function that serves as a score function in the `online_binpack` function.                                                                                                             
3. The heuristic only takes two inputs: 
    item: float, size of item to be added to the bin
    bins: numpy array, an array of capacities for each bin                                                                       
4. Two example heuristics are listed below as <|example heuristic 1|> and <|example heuristic 2|>. You should revise the example heuristics and propose a better one
4. Only generate one new heuristic at a time.                                            

                                                                                                                                                                                                                                                                                                                                                                                     
<|evaluation function|>                                                      
```python 
def evaluate(instances: dict, online_binpack, priority) -> float:
...
def update_function(code: str, instances: dict, database:dict, num_iter: int, eval_fun, parents_code: list) -> dict:
    ...
    online_binpack = getattr(module, 'online_binpack')

    # compute score
    opt_num_bins = get_opt_num_bins(instances)
    avg_num_bins = -eval_fun(instances, online_binpack, priority)
